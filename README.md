...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`pulldown-cmark`](https://crates.io/crates/pulldown-cmark).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `pulldown-cmark` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

CommonMark + extensions parser
...
Original work copyrighted by the upstream `pulldown-cmark` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
