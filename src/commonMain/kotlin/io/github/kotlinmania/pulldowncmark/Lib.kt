// port-lint: source pulldown-cmark/src/lib.rs
...
public enum class HeadingLevel(
    ...
    public companion object {
        public fun from(value: Int): HeadingLevel? =
            when (value) {
                1 -> H1
                2 -> H2
                3 -> H3
...
public class InvalidHeadingLevel(
    public val value: Int,
) : Exception("Invalid heading level: $value")

public sealed class CodeBlockKind {
    public data object Indented : CodeBlockKind()

    public data class Fenced(
        public val info: CowStr,
    ) : CodeBlockKind()
...
public sealed class TagEnd {
    ...
    public data class MetadataBlock(
        public val kind: MetadataBlockKind,
    ) : TagEnd()
}
...
public sealed class Tag {
    ...
    public data class CodeBlock(
        public val kind: CodeBlockKind,
    ) : Tag()

    public data object HtmlBlock : Tag()

    public data class List(
        public val startNumber: Long? = null,
    ) : Tag()
    ...
    public data class Table(
        public val alignments: kotlin.collections.List<Alignment>,
    ) : Tag()
...
public sealed class Event {
    ...
    public data class End(
        public val tagEnd: TagEnd,
    ) : Event()

    public data class Text(
        public val text: CowStr,
    ) : Event() {
    ...
    public data class InlineHtml(
        public val text: CowStr,
    ) : Event() {
        public constructor(s: String) : this(CowStr(s))
    }

    public data class FootnoteReference(
        public val label: CowStr,
    ) : Event() {
        public constructor(s: String) : this(CowStr(s))
    }
    ...
    public data class TaskListMarker(
