# Claude Code Project Instructions — pulldown-cmark-kotlin

## Project Overview

This is **pulldown-cmark-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`pulldown-cmark`](https://crates.io/crates/pulldown-cmark).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/pulldown-cmark/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/pulldown-cmark>
